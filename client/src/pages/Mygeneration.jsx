      ...
      <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div className="rounded-2xl bg-white/6 border border-white/10 animate-pulse h-[260px]"></div>
            ))}
          </div>
        )}
        {/* empty state */}
        {!loading && thumbnails.length === 0 && (
          <div className="text-center py-24">
            <h3 className="tex-lg font-semibold text-zinc-200">
              No thumbnails yet
            </h3>
        ...
        {!loading && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
            {thumbnails.map((thumb) => {
              ...
              return (
                <div
                  className="h-full mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid"
                  key={thumb._id}
                ...
                >
                  ...
                  >
                    <TrashIcon
                      ...
                      className="size-6 p-1 bg-black/50 rounded hover:bg-pink-600 transition-all"
                    />
                    <DownloadIcon
                    ...
                    >
                      <ArrowRightIcon className="size-6 p-1 bg-black/50 rounded hover:bg-pink-600 transition-all" />
                    </Link>
                  </div>
                </div>
              );
